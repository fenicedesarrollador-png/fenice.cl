...
import { buildMetadata, comunaKeywords, jsonLd } from "@/lib/seo";
...
export function buildComunaMetadata(c: ComunaConfig): Metadata {
  ...
  return buildMetadata({
    title: `Petróleo a Domicilio en ${c.nombre}`,
    description: `Despacho de diésel para empresas, generadores y maquinaria en ${c.nombre}, ${region}. Cotiza indicando litros, dirección y fecha de entrega.`,
      ...
      <section className="py-14 bg-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-gray-600 leading-relaxed mb-6">
            Desde nuestra base en San Ramón coordinamos entregas en {config.nombre} según ruta,
            disponibilidad y condiciones de acceso. Confirma por WhatsApp el volumen, dirección y fecha
            requerida; emitimos factura electrónica para tu departamento de compras.
          </p>

          <h2 className="text-2xl font-bold text-gray-900 mb-4 mt-8">
            Operaciones que pueden requerir combustible en {config.nombre}
          </h2>
          <p className="text-gray-600 mb-4">
            {config.nombre} tiene un perfil predominantemente {config.perfil}. Entre las necesidades
            frecuentes de abastecimiento de esta zona se encuentran:
          </p>
          ...
          <p className="text-gray-600 mb-6 leading-relaxed">
            ...
            Confirmamos precio y disponibilidad dentro del horario Lun-Vie 09:00–19:00.
          </p>

          <div className="bg-slate-50 border border-slate-200 rounded-xl p-6 mb-8">
            <h3 className="font-bold text-[#0a1628] mb-3">Datos necesarios para confirmar el despacho</h3>
            <ul className="grid sm:grid-cols-2 gap-2 text-sm text-slate-600">
              <li>• Cantidad de litros requerida</li>
              <li>• Dirección exacta en {config.nombre}</li>
              <li>• Fecha y franja horaria preferida</li>
              <li>• Tipo de estanque, equipo o punto de descarga</li>
              <li>• Restricciones de acceso para el camión</li>
              <li>• Datos de facturación de la empresa</li>
            </ul>
          </div>
          ...
          <div className="bg-[#ecfdf3] border border-[#1a6b3c]/15 rounded-xl p-6 mb-8">
            ...
            <ul className="space-y-2">
              {SERVICIOS.map((s) => (
                <li key={s.slug}>
                  <Link href={s.href} className="text-[#1a6b3c] hover:text-[#145530] hover:underline font-semibold">
                    {s.nombre}
                  </Link>
                </li>
              ))}
