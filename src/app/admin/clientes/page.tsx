  ...
  return (
    <div className="p-4 sm:p-6 lg:p-7 max-w-[1200px] mx-auto admin-rise">
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        {[
          { label: "Total", value: clientes.length, color: "text-[#e8eef7]" },
          { label: "Activos", value: activos, color: "text-[#2bbe6a]" },
          { label: "Con testimonio", value: clientes.filter((c) => c.testimonio).length, color: "text-[#f5a623]" },
        ].map((s) => (
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
          {clientes.map((c) => (
            <div key={c.id} className={`admin-card overflow-hidden group transition-all hover:shadow-md ${!c.activo ? "opacity-60" : ""}`}>
              <div className="bg-white/[0.03]/80 h-24 flex items-center justify-center px-6 relative border-b border-white/[0.06]">
                ...
                {c.logo_url ? (
                  ...
                  <div className="w-14 h-14 rounded-xl bg-[#2bbe6a]/12 flex items-center justify-center"><span className="text-lg font-black text-[#2bbe6a]">{c.nombre.slice(0, 2).toUpperCase()}</span></div>
                )}
              </div>
              <div className="p-3.5">
                <div className="flex items-start justify-between gap-2">
                  <h3 className="font-black text-[#e8eef7] text-sm leading-snug">{c.nombre}</h3>
                  <span className="text-[10px] text-[#5f739a] font-bold shrink-0">#{c.orden}</span>
                </div>
                {c.sector && <span className="inline-block text-[10px] bg-white/8 text-[#cdd9ea] font-bold px-1.5 py-0.5 rounded-md mt-1.5">{c.sector}</span>}
                {c.testimonio && (
                  <div className="flex items-start gap-1.5 mt-2">
                    <Star className="w-3 h-3 text-[#f5a623] fill-[#f5a623] shrink-0 mt-0.5" />
                    <p className="text-[11px] text-[#94a7c2] leading-relaxed line-clamp-2">{c.testimonio}</p>
                  </div>
                )}
                <div className="flex items-center gap-1.5 pt-3 mt-3 border-t border-white/[0.06]">
                  <Link href={`/admin/clientes/${c.id}/editar`} className="flex-1 inline-flex items-center justify-center gap-1.5 text-[12px] font-bold text-[#cdd9ea] hover:text-[#2bbe6a] py-2 rounded-xl border border-white/12 hover:border-[#2bbe6a]/40 hover:bg-[#2bbe6a]/12 transition-all"><Edit2 className="w-3 h-3" />Editar</Link>
