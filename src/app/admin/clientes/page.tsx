    ...
    <div className="p-4 sm:p-6 lg:p-7 max-w-[1200px] mx-auto admin-rise">
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        {[
          ...
          { label: "Con testimonio", value: clientes.filter((c) => c.testimonio).length, color: "text-[#b87608]" },
        ].map((s) => (
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
          {clientes.map((c) => (
            <div key={c.id} className={`admin-card overflow-hidden group transition-all hover:shadow-md ${!c.activo ? "opacity-60" : ""}`}>
              <div className="bg-slate-50 h-24 flex items-center justify-center px-6 relative border-b border-slate-100">
                ...
                {c.logo_url ? (
                  ...
                  <div className="w-14 h-14 rounded-xl bg-[#1a6b3c]/12 flex items-center justify-center"><span className="text-lg font-black text-[#1a6b3c]">{c.nombre.slice(0, 2).toUpperCase()}</span></div>
                )}
              </div>
              <div className="p-3.5">
                <div className="flex items-start justify-between gap-2">
                  <h3 className="font-black text-[#0a1628] text-sm leading-snug">{c.nombre}</h3>
                  <span className="text-[10px] text-slate-400 font-bold shrink-0">#{c.orden}</span>
                </div>
                {c.sector && <span className="inline-block text-[10px] bg-slate-100 text-slate-700 font-bold px-1.5 py-0.5 rounded-md mt-1.5">{c.sector}</span>}
                {c.testimonio && (
                  <div className="flex items-start gap-1.5 mt-2">
                    <Star className="w-3 h-3 text-[#b87608] fill-[#f5a623] shrink-0 mt-0.5" />
                ...
                <div className="flex items-center gap-1.5 pt-3 mt-3 border-t border-slate-100">
                  <Link href={`/admin/clientes/${c.id}/editar`} className="flex-1 inline-flex items-center justify-center gap-1.5 text-[12px] font-bold text-slate-700 hover:text-[#1a6b3c] py-2 rounded-xl border border-slate-200 hover:border-[#1a6b3c]/40 hover:bg-[#1a6b3c]/12 transition-all"><Edit2 className="w-3 h-3" />Editar</Link>
