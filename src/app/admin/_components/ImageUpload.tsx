...
export default function ImageUpload({ bucket, name, defaultUrl = "", label = "Imagen" }: Props) {
  ...
  return (
    <div>
      ...
      {url ? (
        <div className="relative group rounded-xl overflow-hidden border border-white/12">
          ...
          <div className="absolute inset-0 bg-[#0a1628]/0 group-hover:bg-[#0a1628]/40 transition-colors flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100">
            <button type="button" onClick={() => inputRef.current?.click()} className="bg-white/[0.04] text-[#e8eef7] text-xs font-bold px-3 py-1.5 rounded-lg">Cambiar</button>
        ...
        <div
          ...
          className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all ${drag ? "border-[#1a6b3c] bg-[#2bbe6a]/12" : "border-white/12 hover:border-[#1a6b3c]/40 hover:bg-white/[0.05]"}`}
        >
          {uploading ? (
            <div className="flex flex-col items-center gap-2">
              <Loader2 className="w-6 h-6 text-[#2bbe6a] animate-spin" />
              <p className="text-sm text-[#94a7c2] font-medium">Subiendo imagen…</p>
            </div>
          ) : (
            <div className="flex flex-col items-center gap-2">
              <div className="w-11 h-11 rounded-xl bg-[#2bbe6a]/12 flex items-center justify-center">
                <UploadCloud className="w-5 h-5 text-[#2bbe6a]" />
              </div>
              <p className="text-sm text-[#cdd9ea] font-semibold">Arrastra o haz clic para subir</p>
              <p className="text-[11px] text-[#94a7c2]">JPG, PNG o WebP · máximo 2 MB</p>
            </div>
          )}
        </div>
      )}
      ...
      {error && <p className="text-[11px] text-red-400 mt-1.5 font-medium flex items-center gap-1"><X className="w-3 h-3" />{error}</p>}
    </div>
  );
}
