...
export default function ImageUpload({ bucket, name, defaultUrl = "", label = "Imagen" }: Props) {
  ...
  return (
    <div>
      ...
      {url ? (
        <div className="relative group rounded-xl overflow-hidden border border-slate-200">
          ...
          <div className="absolute inset-0 bg-[#0a1628]/0 group-hover:bg-[#0a1628]/40 transition-colors flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100">
            <button type="button" onClick={() => inputRef.current?.click()} className="bg-slate-50 text-[#0a1628] text-xs font-bold px-3 py-1.5 rounded-lg">Cambiar</button>
        ...
        <div
          ...
          className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all ${drag ? "border-[#1a6b3c] bg-[#1a6b3c]/12" : "border-slate-200 hover:border-[#1a6b3c]/40 hover:bg-slate-100"}`}
        >
          {uploading ? (
            ...
            <div className="flex flex-col items-center gap-2">
              <div className="w-11 h-11 rounded-xl bg-[#1a6b3c]/12 flex items-center justify-center">
              ...
              <p className="text-[11px] text-slate-500">JPG, PNG o WebP · máximo 2 MB</p>
            </div>
          )}
        </div>
      )}
      ...
      {error && <p className="text-[11px] text-red-500 mt-1.5 font-medium flex items-center gap-1"><X className="w-3 h-3" />{error}</p>}
    </div>
  );
}
