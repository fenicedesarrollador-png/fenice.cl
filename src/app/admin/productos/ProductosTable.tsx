    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          { label: "Destacados", value: destacados, color: "text-[#b87608]" },
        ].map((s) => (
      ...
      <div className="flex flex-col sm:flex-row gap-2.5">
        ...
        <div className="flex gap-2">
          {(["todos", "activos", "inactivos"] as const).map((f) => (
            <button key={f} onClick={() => setFilterActivo(f)} className={`px-3.5 py-2 rounded-xl text-[12px] font-bold transition-all border capitalize ${filterActivo === f ? "bg-[#1a6b3c] text-white border-[#1a6b3c] shadow-sm shadow-[#1a6b3c]/20" : "bg-slate-50 border-slate-200 text-slate-700 hover:border-slate-300"}`}>
              {f}
            </button>
          ))}
        </div>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-16 admin-card border-dashed">
          <div className="w-12 h-12 rounded-2xl bg-slate-50 flex items-center justify-center mx-auto mb-3"><Package className="w-6 h-6 text-slate-400" /></div>
          <p className="text-[#0a1628] font-bold text-sm">{productos.length === 0 ? "Aún no hay productos" : "Sin resultados"}</p>
          <p className="text-slate-500 text-xs mt-1">{productos.length === 0 ? "Crea tu primer producto del catálogo." : "Ajusta la búsqueda o filtros."}</p>
            ...
            <div key={p.id} className={`admin-card overflow-hidden group transition-all hover:shadow-md ${!p.activo ? "opacity-70" : ""}`}>
              <div className="flex items-center gap-3 p-3.5">
                {p.imagen_url ? (
                  ...
                  <div className="w-14 h-14 rounded-xl bg-slate-50 flex items-center justify-center shrink-0"><Package className="w-5 h-5 text-slate-400" /></div>
                )}
                <div className="flex-1 min-w-0">
                  <p className="font-black text-[#0a1628] text-sm truncate">{p.nombre}</p>
                  <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                    {p.categoria && <span className="text-[10px] bg-slate-100 text-slate-700 font-bold px-1.5 py-0.5 rounded-md">{p.categoria}</span>}
                    {p.destacado && <span className="inline-flex items-center gap-0.5 text-[10px] text-[#b87608] font-bold"><Star className="w-2.5 h-2.5 fill-current" />Destacado</span>}
                  </div>
                </div>
              </div>
              <div className="px-3.5 py-2.5 border-t border-slate-100 flex items-center gap-1.5">
                <button onClick={() => toggleActivo(p.id, p.activo)} disabled={toggling === p.id} className={`inline-flex items-center gap-1 text-[11px] font-bold px-2 py-1.5 rounded-lg border transition-all disabled:opacity-50 ${p.activo ? "bg-[#1a6b3c]/12 text-[#1a6b3c] border-[#1a6b3c]/20" : "bg-slate-50 text-slate-500 border-slate-200"}`}>
                  {p.activo ? <ToggleRight className="w-3.5 h-3.5" /> : <ToggleLeft className="w-3.5 h-3.5" />}
                  {p.activo ? "Activo" : "Inactivo"}
                </button>
                <div className="ml-auto flex items-center gap-1">
                  <Link href={`/admin/productos/${p.id}/editar`} className="p-1.5 rounded-lg text-slate-500 hover:text-[#1a6b3c] hover:bg-[#1a6b3c]/12 transition-all" title="Editar"><Edit2 className="w-3.5 h-3.5" /></Link>
                  <a href={`/productos/${p.slug}`} target="_blank" rel="noopener noreferrer" className="p-1.5 rounded-lg text-slate-500 hover:text-slate-700 hover:bg-slate-100 transition-all" title="Ver"><ArrowUpRight className="w-3.5 h-3.5" /></a>
                  {confirmDel === p.id ? (
                    <div className="flex items-center gap-1 bg-red-50 border border-red-200 rounded-lg px-2 py-1">
                      ...
                      <button onClick={() => setConfirmDel(null)} className="text-slate-500"><X className="w-3 h-3" /></button>
                    </div>
                  ) : (
                    <button onClick={() => setConfirmDel(p.id)} className="p-1.5 rounded-lg text-slate-500 hover:text-red-500 hover:bg-red-50 transition-all" title="Eliminar"><Trash2 className="w-3.5 h-3.5" /></button>
                  )}
                </div>
              </div>
            </div>
