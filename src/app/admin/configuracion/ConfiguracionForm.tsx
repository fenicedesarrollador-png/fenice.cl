...
export default function ConfiguracionForm({ config }: { config: Record<string, string> }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <FormSection title="Páginas que se actualizan" description="Estas rutas se revalidan automáticamente al guardar.">
        <div className="flex flex-wrap gap-2">
          {PATHS_TO_REVALIDATE.map((path) => (
            <a key={path} href={path} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-[11px] bg-slate-50 border border-slate-200 hover:border-[#1a6b3c]/40 text-slate-700 hover:text-[#1a6b3c] px-2.5 py-1 rounded-lg transition-all font-medium">
      ...
      {success && (
        <div className="flex items-center gap-3 bg-[#1a6b3c]/12 border border-[#1a6b3c]/20 rounded-xl px-5 py-4 text-[#1a6b3c]">
