...
const AVATAR = [
  "bg-[#ecfdf3] text-[#1a6b3c]",
  "bg-[#fff7ec] text-[#d98a0e]",
    ...
    <div className="p-4 sm:p-6 lg:p-7 max-w-[1000px] mx-auto admin-rise">
      ...
      <div className="space-y-2.5">
            ...
            <div key={p.id} className={`admin-card p-3.5 flex items-center gap-3 transition-all hover:shadow-md ${!p.activo ? "opacity-60" : ""}`}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {isSelf && <span className="text-[10px] bg-[#fff7ec] text-[#d98a0e] font-black px-1.5 py-0.5 rounded-full shrink-0">Tú</span>}
                </div>
                <p className="text-[11px] text-slate-400 truncate">{email}</p>
                {last && <p className="text-[10px] text-slate-400 mt-0.5">Último acceso: {new Date(last).toLocaleDateString("es-CL", { dateStyle: "medium" })}</p>}
              </div>
              ...
              <div className="flex items-center gap-1 shrink-0">
                <Link href={`/admin/usuarios/${p.id}/editar`} className="text-[12px] font-bold text-slate-600 hover:text-[#1a6b3c] px-2.5 py-1.5 rounded-lg border border-slate-200 hover:border-[#1a6b3c]/30 hover:bg-[#ecfdf3] transition-all">Editar</Link>
                {!isSelf && (
                  <>
        ...
        {(!profiles || profiles.length === 0) && (
          <div className="text-center py-16 admin-card border-dashed text-slate-400 text-sm">No hay usuarios registrados.</div>
        )}
      </div>
    </div>
  );
