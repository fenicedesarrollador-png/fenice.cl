      ...
      <div className="relative w-full max-w-xl admin-card !rounded-2xl overflow-hidden shadow-2xl admin-rise">
        ...
        <div className="flex items-center gap-3 px-4 py-3.5 border-b border-slate-100">
          ...
          <input
            ...
            className="flex-1 bg-transparent text-[15px] text-[#0a1628] placeholder:text-slate-400 outline-none"
          />
            ...
            results.map((cmd, i) => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-2.5 text-left transition-colors ${
                  i === active ? "bg-[#1a6b3c]/12" : "hover:bg-slate-50"
                }`}
              >
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
                  i === active ? "bg-[#1a6b3c]/20 text-[#1a6b3c]" : "bg-slate-100 text-slate-500"
                }`}>
                  <cmd.icon className="w-4 h-4" />
                </div>
                <span className="flex-1 min-w-0">
                  <span className="block text-[13.5px] font-semibold text-[#0a1628] truncate">{cmd.label}</span>
                  {cmd.hint && <span className="block text-[11px] text-slate-400">{cmd.hint}</span>}
                </span>
                {i === active && <CornerDownLeft className="w-3.5 h-3.5 text-[#1a6b3c] shrink-0" />}
              </button>
            ))
          )}
        </div>
